...
** =[path-case]= --- Path found only by folding case

*** Message

: [path-case] sigfic.lsp:12 in LOAD-DIAL: LOAD "dial/sigfic.lsp": found as "DIAL/SIGFIC.LSP" -- the case differs, and only a case-insensitive filesystem forgives it

The part after the location is built from the template
=~A ~S: found as ~S -- the case differs, and only a case-insensitive filesystem forgives it=, whose arguments are: WHO (the builtin name, e.g. "LOAD"), the path as written, the real name.
...
: [vl-catch-all-apply-arity] in LOAD-VLE: (vl-catch-all-apply f) without an argument list works on AutoCAD and BricsCAD but is documented by neither; --dialect strict asks for the documented subset. Pass nil explicitly.

The part after the location is built from the template
=(vl-catch-all-apply f) without an argument list works on AutoCAD and BricsCAD but is documented by neither; --dialect strict asks for the documented subset. Pass nil explicitly.=, whose arguments are: none.
...
** =[entmakex-invalid-insert]= --- ENTMAKE / ENTMAKEX of an INSERT referencing an undefined block

*** Message

: [entmakex-invalid-insert] cador accepted an INSERT rejected by AutoCAD and BricsCAD: block SCHME_TEST_C1 is undefined (its group-2 name resolves to no BLOCK_RECORD); the create returns nil under the autocad and bricscad dialects.

The part after the location is built from the template
=cador accepted an INSERT rejected by AutoCAD and BricsCAD: its group-2 block is undefined. The create returns nil under the autocad and bricscad dialects.=, whose arguments are: the group-2 block name.
...
** =[foreach-empty-body]= --- FOREACH with no body — return value

*** Message

: [foreach-empty-body] a FOREACH with no body returns nil under the specification and AutoCAD, but BricsCAD returns the list left in its accumulator. clautolisp returns the list under --dialect bricscad; the two answers are not portable.

The part after the location is built from the template
=a FOREACH with no body returns nil under the spec and AutoCAD, but the list under BricsCAD=, whose arguments are: the active dialect and which value clautolisp is about to return.
...
** =[read-leading-comment]= --- READ on a string starting with a comment

*** Message

: [read-leading-comment] (read ...) on a string starting with a line comment returns NIL immediately, matching the real engine under --dialect bricscad.
