...
- In strict mode, the only guaranteed portable embedded quote syntax is a doubled double quote inside a string.
- In strict mode, unknown backslash escapes shall not be given implicit Common Lisp meaning.
- In lax mode, additional implementation-defined escape acceptance may be enabled, but such behavior shall be tagged as implementation-defined and kept separate from strict behavior.
