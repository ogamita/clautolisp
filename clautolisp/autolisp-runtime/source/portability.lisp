...
(register-dialect-warning
 :tag "entmakex-invalid-insert"
 :title "ENTMAKE / ENTMAKEX of an INSERT referencing an undefined block"
 :kind :portability
 :message "cador accepted an INSERT rejected by AutoCAD and BricsCAD: its group-2 block is undefined. The create returns nil under the autocad and bricscad dialects."
 :arguments "the group-2 block name"
 :example ": [entmakex-invalid-insert] cador accepted an INSERT rejected by AutoCAD and BricsCAD: block SCHME_TEST_C1 is undefined (its group-2 name resolves to no BLOCK_RECORD); the create returns nil under the autocad and bricscad dialects."
 :dialects
 "An INSERT whose group-2 block name does not resolve to a BLOCK_RECORD is
rejected by BOTH AutoCAD and BricsCAD: entmake / entmakex return nil and
nothing is added to the database (a block reference must reference a block).
This is NOT a vendor divergence -- the two engines agree. Evidence: SCHME+ C1
made cador pass on an INSERT to the undefined block SCHME_TEST_C1 while both
real-CAD jobs failed (AutoCAD Fabrik job 511433, BricsCAD job 511434).

cador historically kept a permissive construction mode that accepted the
invalid INSERT, which hid invalid DXF fixtures until the suite reached a real
CAD. The per-dialect resolution:

- --dialect autocad-* / bricscad-* -- REJECT (return nil), matching the
  vendor; silent.
- --dialect clautolisp -- keep the permissive mode (return the ename), and
  WARN: the acceptance is a cador leniency, not portable.
- --strict -- accept but WARN (a divergence of any kind is unsafe to rely on).
- --lax -- accept, silent."
 :rationale
 "A DXF fixture or production path that entmakes an INSERT to a block it never
defined is broken everywhere except cador's permissive mode; catching it at
create time -- by rejecting under the vendor dialects and warning under
clautolisp/strict -- stops the error hiding until a real CAD job fails. The
immediately checkable invariant is that the group-2 name resolves to a
BLOCK_RECORD; group 66 announcing an attribute sequence that no ATTRIB/SEQEND
completes is the related, deferred check."
 :references
 '(("clautolisp: emitted by" . "clautolisp/cador/source/entity-api.lisp, EMIT-ENTMAKEX-INVALID-INSERT-WARNING")
   ("clautolisp: issue" . "issues/closed/cador-entmakex-invalid-insert-dialect-warning.issue")
   ("clautolisp: normative rules" . "autolisp-spec chapter 25, \"Normative Rules: Dialect Portability Warnings\"")))
...
(register-dialect-warning
 ...
 :kind :vendor-divergence
 :message ""
 :arguments "varies by call site -- comment kind (line / block), the active dialect, and what this call is about to return"
 :example ": [read-leading-comment] (read ...) on a string starting with a line comment returns NIL immediately, matching the real engine under --dialect bricscad."
 :dialects
 "Neither vendor's `read' reference page documents comment-handling in
its string argument at all (Autodesk's and Bricsys' pages are both
silent on it), and the two real engines do not even agree with EACH
OTHER -- confirmed live against real installs, not inferred:

  (read \"; foo\\nbar\")     -- a leading `;' LINE comment
  (read \";| foo |; bar\")   -- a leading `;| ... |;' BLOCK comment

- BricsCAD (V25 Windows, V26 macOS) returns NIL for BOTH forms: the
  instant the first non-blank content it meets is any comment, before
  any real object, it behaves as if EOF were reached immediately.
- AutoCAD 2022 (Windows) returns NIL only for the BLOCK form; a
  leading LINE comment is skipped and the following object returned --
  the same as clautolisp's own reader.
- clautolisp's own reader is uniform and lenient: it always skips a
  leading comment, of either kind, and returns the following object.

So `--dialect bricscad' reproduces BricsCAD's NIL for both kinds (and
warns for both); `--dialect autocad' reproduces AutoCAD's NIL for a
leading BLOCK comment only (and warns only for that kind -- a leading
LINE comment stays silent, since clautolisp already matches AutoCAD
there); `--strict' KEEPS clautolisp's lenient skip-and-return behaviour
for both kinds but WARNS for both, because strict names no vendor and
at least one real engine disagrees either way; `--dialect clautolisp'
and `--lax' stay silent for both -- the same deliberate convenience
already granted to every other clautolisp-only leniency."
 :rationale
 "A caller who wrote or generated a leading comment -- a `;;;;' file
header, or an `;| ... |;' doc block -- and then handed the whole text
to `read' as a STRING (rather than `load'ing it as a file) gets a
silently different answer depending on which real engine runs it: an
object on AutoCAD, NIL on BricsCAD, for the identical string. The
warning surfaces that before it becomes a `nil where NAME was' bug
several calls later."
 :references
 '(("clautolisp: the governing issue" . "issues/open/read-string-does-not-skip-comments.issue")
   ("clautolisp: emitted by" . "clautolisp/autolisp-builtins-core/source/api.lisp, EMIT-READ-LEADING-COMMENT-WARNING")
   ("Autodesk: read (AutoLISP), 2026 CHS" . "https://help.autodesk.com/cloudhelp/2026/CHS/AutoCAD-MAC-AutoLISP-Reference/files/GUID-5B50BB3E-C244-46E8-85D8-6A2D48B1FE51.htm")
   ("Bricsys: read (DevRef V26)" . "https://developer.bricsys.com/bricscad/help/en_US/V26/DevRef/source/read.htm")))
