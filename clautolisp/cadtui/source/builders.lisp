...
;;;; reduced to /application/console (no barre-menu, empty dessins[]) — cador is
...
(empty) barre-menu and the application console. dessins[] is empty until a
drawing is opened (Phase 4). Returns the root UI-APPLICATION."
  (let ((app (make-instance 'ui-application :key "application")))
    (add-child app (make-instance 'ui-menubar :key "barre-menu"))
    (add-child app (make-instance 'ui-console :key "console"))
    (setf (ui-menubar-slot app) (ui-find-child app "barre-menu")
...
only the application console — no barre-menu, empty dessins[]. Returns the root
UI-APPLICATION."
  (let ((app (make-instance 'ui-application :key "application")))
    (add-child app (make-instance 'ui-console :key "console"))
