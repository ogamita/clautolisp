...
#+AUTHOR: Codex
#+OPTIONS: toc:3 num:t tex:verbatim ^:{}
#+TEXINFO_FILENAME: specification-resolution-plan.info
#+TEXINFO_DIR_CATEGORY: AutoLISP specification
#+TEXINFO_DIR_TITLE: specification-resolution-plan: (specification-resolution-plan).
#+TEXINFO_DIR_DESC: Resolution plan for the AutoLISP / Visual LISP spec.
...
* Probe-Suite Findings (BricsCAD V26 / macOS, 2026-04-26)
The probe run captured the following normative findings (see [[file:../results/bricscad/macos/20260426T122808Z/results.sexp][~results.sexp~]]):
...
- *prin1* / *princ* / *print* — all accept the 2-argument form `(fn expr stream)`. Verbatim representations confirmed: `prin1` writes the round-trippable form (with quotes for strings), `princ` writes the value verbatim, `print` writes `\n` + prin1 + ` `.
