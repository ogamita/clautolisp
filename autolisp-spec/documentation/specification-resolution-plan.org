...
- Run `scripts/run-probes.sh` against a real AutoCAD installation when one becomes available, with `RUNNER_TEMPLATE` pointing at the host's batch invocation. AutoCAD product-test parity is the only open Phase-5 follow-up.
